...
public class PlayerDaggerShoots : MonoBehaviour {
    ...
    public void fire() {
        if (timer >= GAME.playerdagger_aspd) {
			...
			temp.GetComponent<MoveDir> ().setLifeTime (GAME.playerdagger_lifetime);
			temp.GetComponentInChildren<TakeDamage> ().SetDamage ( Mathf.CeilToInt( (GAME.WEAP_DAMAGE [1, pa.GetWeapLevel (1)] * (1 /*can be modified to be above???*/ - ResitanceCalculator.Instance.GetResistanceModifier(1))) ));
